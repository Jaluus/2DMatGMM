...
class DetectionWrapper:
    ...
    def __call__(self, input):
        image = input[0]["image"].permute(1, 2, 0).numpy()
        detected_flakes = self.detector.detect_flakes(image)
        instances = DetectionWrapper.detections_to_instances(detected_flakes)

        # currently only supports single batches
        output = [{"instances": instances}]
        return output

    @staticmethod
    def detections_to_instances(detected_flakes):
        pred_masks = []
        scores = []
        pred_classes = []
        for detected_flake in detected_flakes:
            pred_mask = np.array(detected_flake.mask)
            pred_masks.append(pred_mask)
            scores.append(1 - detected_flake.false_positive_probability)
            pred_classes.append(int(detected_flake.thickness) - 1)

        # create a len(detected_flakes) x 4 tensor full of 0.0
        pred_boxes = Boxes(torch.zeros((len(pred_classes), 4), dtype=torch.float32))
...
def evaluate_on_dataset(model, dataset_name, output_dir):
    ...
    mapper = DatasetMapper(
        is_train=False,
        augmentations=[],  # no need to augment test-time data
        image_format="BGR",
    )

    test_loader = build_detection_test_loader(dataset=dataset, mapper=mapper)
